...
void Prediction::on_initialize()
{
    this->ball = EcsManager::find_entity("Ball")->get_component<Transform>();
...
double Prediction::v(const double t, const double vx, const double vy) const
{
    return (vy - g * t / abs(vx)) / abs(vx);
}

// zero crossing of v(t)
double Prediction::zero_crossing_v(const double vx, const double vy) const
{
    return abs(vx) * vy / g;
}

// f(t) = ∫v(t)dt = (vyt-gt^2/2vx)/vx+c
double Prediction::f(const double t, const double vx, const double vy, const double c) const
{
    return (-g * t * t / (2 * abs(vx)) + vy * t) / abs(vx) + c;
}

// positive zero crossing of f(t)
double Prediction::pos_zero_crossing_f(const double vx, const double vy, const double py) const
{
...
// x1,2 = (-b±√(b^2-4ac)) / 2a
double Prediction::pos_zero_crossing(const double a, const double b, const double c)
{
...
double Prediction::length(const double x, const double a, const double b)
{
...
// l = ∫[0,pzcf](√(1+v(t)^2))
double Prediction::length_f(const double x, const double vx, const double vy) const
{
...
double Prediction::x_from_length_f(const double l, const double vx, const double vy, const double py) const
{
