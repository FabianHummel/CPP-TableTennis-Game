...
Transform::Transform(const Vector3& position, const Vector2Int& scale, const SDL_FPoint &anchor, float angle, int zIndex)
	: position(position), scale(scale), anchor(anchor), rotation(angle), zIndex(zIndex) {}

Transform::Transform()
	: position(0, 0, 0), scale(0, 0), anchor(), rotation(0.0f), zIndex(0) {}
