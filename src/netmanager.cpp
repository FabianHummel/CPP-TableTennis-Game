...
bool NetManager::start(const char *match_code)
{
	...
	host = enet_host_create(&hostAddress, 1, 1, 0, 0);
	if (host == nullptr)
	{
	...
	enet_uint32 serverData;
	if (match_code != nullptr && strlen(match_code) > 0)
	{
		const unsigned int encoded = strtoul(match_code, nullptr, 36);
		serverData = encoded | JOIN_MATCH << 26;
	}
	else
	{
		serverData = CREATE_MATCH << 26;
	}

	if (!enet_host_connect(host, &serverAddress, 1, serverData))
	{
...
void NetManager::update(double deltaTime)
{
	if (host == nullptr)
	{
		return;
