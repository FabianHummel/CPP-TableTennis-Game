...
GamePane::GamePane(RenderWindow *window) : Pane(window)
{
	...
	net = (new Entity("Net"))
	          ...
	          ->apply({RenderWindow::SCREEN_CENTER_X, 0, RenderWindow::SCREEN_CENTER_Y - 100},
	                  {RenderWindow::SCREEN_WIDTH, 129}, {0.5f, 0.5f}, 0.0f, RenderIndexes::NET);

	shadow = (new Entity("Shadow"))
	             ->getTransform()
	             ->apply({0, 0, 0}, {40, 40}, {0.5f, 0.5f}, 0.0f, RenderIndexes::DEFAULT)
