...
GamePane::GamePane(SDL_Renderer *renderer) : Pane(renderer)
{
	table = EcsManager::addEntity(new Entity("Table"))
		...
		->addComponent(new SpriteRenderer("res/table.png", renderer));
	...
	net = EcsManager::addEntity(new Entity("Net"))
		...
		->addComponent(new SpriteRenderer("res/net.png", renderer));

	shadow = EcsManager::addEntity(new Entity("Shadow"))
		->transform
		->apply({0, 0, 0}, {40, 40}, {0.5f, 0.0f}, 0.0f, RenderIndexes::Game::DEFAULT)
		->addComponent(new SpriteRenderer("res/shadow.png", renderer));
	...
	powerbox = EcsManager::addEntity(new Entity("Powerbox"))
		...
		->addComponent(new SpriteRenderer("res/powerbox.png", renderer));
...
void GamePane::onEvent(const SDL_Event *event)
{
	switch (event->type)
	{
