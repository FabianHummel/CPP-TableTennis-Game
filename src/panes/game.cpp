...
GamePane::GamePane(RenderWindow *window) : Pane(window) {
  ...
  net = (new Entity("Net"))
            ...
            ->getTransform()
            ->apply({RenderWindow::SCREEN_CENTER_X, 0,
                     RenderWindow::SCREEN_CENTER_Y - 100},
                    {RenderWindow::SCREEN_WIDTH, 129}, {0.5f, 0.5f}, 0.0f,
                    RenderIndexes::NET);

  shadow = (new Entity("Shadow"))
               ->getTransform()
               ->apply({0, 0, 0}, {40, 40}, {0.5f, 0.5f}, 0.0f,
                       RenderIndexes::DEFAULT)
               ->addComponent(
                   new SpriteRenderer("res/shadow.png", window->renderer));
  ...
  indicator = (new Entity("Indicator"))
                  ->getTransform()
                  ->apply({0, 0, 0}, {112, 112}, {0.5f, 0.5f}, 0.0f,
                          RenderIndexes::UI + 1)
                  ->addComponent(new SpriteRenderer("res/indicator.png",
                                                    window->renderer));

  powerbar = (new Entity("Powerbar"))
                 ->getTransform()
                 ->apply({0, 0, 0}, {70, 70}, {0.5f, 1.0f}, 0.0f,
                         RenderIndexes::UI + 0)
                 ->addComponent(
                     new SpriteRenderer("res/powerbar.png", window->renderer))
                 ->addComponent(new Powerbar());

  powerbox = (new Entity("Powerbox"))
                 ->getTransform()
                 ->apply({0, 0, 0}, {70, 70}, {0.5f, 0.5f}, 0.0f,
                         RenderIndexes::UI + 1)
                 ->addComponent(
                     new SpriteRenderer("res/powerbox.png", window->renderer));

  prediction =
      (new Entity("Prediction"))
          ->addComponent(new Prediction(window->renderer))
          ->getTransform()
          ->apply({0, 0, 0},
                  {RenderWindow::SCREEN_WIDTH, RenderWindow::SCREEN_HEIGHT},
