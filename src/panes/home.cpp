...
HomePane::HomePane(RenderWindow *window) : Pane(window)
{
	title = (new Entity("Title"))
	            ...
	            ->getTransform()
	            ->apply({RenderWindow::SCREEN_CENTER_X, 0, 425 / 2.0}, {RenderWindow::SCREEN_WIDTH, 425}, {0.5f, 0.5f},
	                    0.0f, RenderIndexes::MENUTITLE);

	background =
		(new Entity("Background"))
			->addComponent(new BubbleDrawer(window->renderer))
			->getTransform()
			->apply({RenderWindow::SCREEN_HEIGHT, 0, 0}, {RenderWindow::SCREEN_WIDTH, RenderWindow::SCREEN_HEIGHT},
	                {0.5f, 0.5f}, -3.8, RenderIndexes::MENUBACKGROUND);

	gamemode = (new Entity("Gamemode"))
	               ->addComponent(new TextRenderer(window->renderer,
	                                               magic_enum::enum_name(GameMode::SINGLEPLAYER).data(), {40, 40, 40}))
	               ->getTransform()
	               ->apply({RenderWindow::SCREEN_CENTER_X, 0, 800}, {0, 0}, {0.5f, 0.5f}, 0.0f, RenderIndexes::UI);
	...
	previous =
		(new Entity("Previous"))
			...
			->getTransform()
			->apply({RenderWindow::SCREEN_CENTER_X - 200, 0, 800}, {40, 40}, {0.5f, 0.5f}, 0.0f, RenderIndexes::UI);
